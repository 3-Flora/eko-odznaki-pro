...
export class DocumentUploadService {
  ...
  static validateFile(file, documentType) {
    ...
    if (file.size > this.MAX_FILE_SIZE) {
      errors.push(
        `Plik jest za duży. Maksymalny rozmiar: ${this.MAX_FILE_SIZE / 1024 / 1024}MB`,
      );
    }
    ...
    if (!this.ALLOWED_TYPES[file.type]) {
      errors.push(
        `Nieprawidłowy typ pliku. Dozwolone: ${Object.values(this.ALLOWED_TYPES).join(", ")}`,
      );
    }
  ...
  static async uploadTeacherDocument(
    ...
    try {
      ...
      const fileName =
        documentType === "idCard" ? "id-card" : "employment-cert";
